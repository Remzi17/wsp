...
dataDa.forEach(item => {
	...
	daMedia.forEach(media => {
		...
		// Функция для обработки сложных селекторов с индексами
		function getTargetElement(selector, context = document) {
			let parts = selector.trim().split(' '); // Разбиваем по пробелам (учитываем вложенность)
			...
			for (let part of parts) {
				...
				if (!currentContext) break; // Прерываем, если не найдено
			}
